...
function createComment(url, request) {
  ...
  const response = {};
  if (requestComment && requestComment.username && database.users[requestComment.username] && requestComment.body && requestComment.articleId && database.articles[requestComment.articleId]) {
    const comment = {
...
function deleteComment (url) {
  ...
  if (savedComment) {
    ...
    removeArrVal(database.articles[savedComment.articleId].commentIds,savedComment.id)
    removeArrVal(database.users[savedComment.username].commentIds, savedComment.id)
    response.status = 204;
  } else {
    response.status = 404;
  }
